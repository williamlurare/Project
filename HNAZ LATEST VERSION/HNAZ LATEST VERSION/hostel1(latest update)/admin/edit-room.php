...
$query="update room set ID_Room = ?,Fees=? where id=?";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('sii',$ID_Room,$Fees,$id);
$stmt->execute();
	 ...
	 //$cnt=1;
	   while($row=$res->fetch_object())
	  {
	  	?>
					...
					<div class="col-sm-8">
					<input type="text"  name="seater" value="<?php echo $row->ID_Room;?>"  class="form-control"> </div>
					</div>
				 <div class="form-group">
				<label class="col-sm-2 control-label">Hostel </label>
		<div class="col-sm-8">
	<input type="text" class="form-control" name="rmno" id="rmno" value="<?php echo $row->ID_Hostel;?>" disabled>
	<span class="help-block m-b-none">
													Hostel can't be changed.</span>
						 </div>
						</div>
<div class="form-group">
									<label class="col-sm-2 control-label">Fees </label>
									<div class="col-sm-8">
									<input type="text" class="form-control" name="fees" value="<?php echo $row->Fees;?>" disabled >
												</div>
											</div>
